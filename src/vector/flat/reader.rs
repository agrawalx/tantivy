...
//! [`VectorColumn`] views for sequential scan.
...
impl FlatVecReader {
    pub(crate) fn open(segment_reader: &SegmentReader) -> crate::Result<Self> {
        ...
        let composite = if field_dims.is_empty() {
            ...
            match segment_reader.open_read(SegmentComponent::Custom(super::FLATVEC_EXT.to_string()))
            {
    ...
    pub(crate) fn has_column(&self, field: Field) -> bool {
        self.field_dims.contains_key(&field)
            && self.composite.open_read_with_idx(field, 0).is_some()
    }
}
